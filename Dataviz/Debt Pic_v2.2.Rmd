...
```{r}
debt_dat <- read.csv("~/Onedrive - Emory University/Fall_SNA/Social Network Analysis - Final Project/crossholding dataset.csv", stringsAsFactors = FALSE)
str(debt_dat)
```
...
```{r}
hist(debt_df$weight, ylim = c(1,60), xlab = 'Debt', main = 'Histogram for weight of edges')
```
...
```{r}
debt_g <- graph_from_data_frame(debt_df[, 1:2], directed = TRUE)
# add edge weights
...
debt_ec <- eigen_centrality(debt_g, directed = TRUE)$vector
...
Calculte the closeness for directed networks
...
data.frame(
  Rank = seq_along(cl), 
  closeness = names((sort(cl, decreasing = TRUE, na.last = TRUE)))
)
...
write.csv(centrality_df, "centrality_rank.csv",row.names=F)
```


Assortivity degree: whether similar points are connected
